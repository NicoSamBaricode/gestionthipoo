...
class actividades_class extends conexionDb
{
	...
	public function borrar_actividad($id)
	{
		...
		$sql = $this->conexion->query($query);
		if ($sql == true) {
			$log->insertarLog($_SESSION['user'],"Se borro el la actividad con id: ".$id);
			echo "<script> alert('Se borraron los datos con exito'); window.location='/GestionThi/gestionthipoo/Lista_actividades.php'</script> ";
		} else {
			$log->insertarLog($_SESSION['user'], "Fallo al borrar una actividad");
	...
	{   $log=new log_class();
		...
		if (!empty($id_a) && !empty($postData)) {
			...
			$sql = $this->conexion->query($query);
			if ($sql == true) {
				$log->insertarLog($_SESSION['user'],"Se modifico la actividad con id: ".$id_a);
				...
				$log->insertarLog($_SESSION['user'], "Fallo al actualizar una actividad");
